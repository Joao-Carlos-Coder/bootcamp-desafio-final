...
class ConditionalEncoder(nn.Module):
    def __init__(self, input_channels, num_classes, latent_dim):
        ...
        # Parte a ser completada pelos alunos:
        # Defina a primeira camada convolucional.
        # Ela deve ter 'input_channels + num_classes' canais de entrada e 32 canais de saída.
        self.conv1 = nn.Conv2d(input_channels + num_classes, 32, kernel_size=4, stride=2, padding=1)  # [batch_size, 32, 14, 14]

        # Parte a ser completada pelos alunos:
        # Defina a segunda camada convolucional.
        # Ela deve ter 32 canais de entrada e 64 canais de saída.
        self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=1)  # [batch_size, 64, 7, 7]

        # Parte a ser completada pelos alunos:
        # Defina a terceira camada convolucional.
        # Ela deve ter 64 canais de entrada e ? de saída.
        self.conv3 = nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1)  # [batch_size, 128, 3, 3]

        # Parte a ser completada pelos alunos:
        # Achate a saída da última camada convolucional para que ela possa ser passada para as camadas lineares.
        self.flatten = nn.Flatten()  # [batch_size, 128*3*3]

        # Parte a ser completada pelos alunos:
        # Defina a camada para a média do espaço latente. 128*3*3
        self.fc_mu = nn.Linear(128*3*3, latent_dim)

        # Parte a ser completada pelos alunos:
        # Defina a camada para a variância logarítmica. 128*3*3
        self.fc_logvar = nn.Linear(128*3*3, latent_dim)

    def forward(self, x, labels):
        # Dica: Redimensione os rótulos para que eles se tornem compatíveis com as dimensões das imagens.
        labels = labels.view(labels.size(0), labels.size(1), 1, 1)

        # Dica: Expanda os rótulos para que tenham o mesmo tamanho que as imagens.
        labels = labels.expand(labels.size(0), labels.size(1), x.size(2), x.size(3))

        # Dica: Concatene as imagens com os rótulos ao longo da dimensão dos canais.
        x = torch.cat((x, labels), dim=1)  # concatenando as labels

        # Dica: Aplique a primeira camada convolucional e a ativação ReLU.
        h = torch.relu(self.conv1(x))

        # Dica: Aplique a segunda camada convolucional e a ativação ReLU.
        h = torch.relu(self.conv2(h))

        # Dica: Aplique a terceira camada convolucional e a ativação ReLU.
        h = torch.relu(self.conv3(h))

        # Dica: Achate a saída antes de passá-la para as camadas lineares.
        h = self.flatten(h)

        # Dica: Obtenha a média do espaço latente.
        mu = self.fc_mu(h)

        # Dica: Obtenha a variância logarítmica.
        logvar = self.fc_logvar(h)

        # Dica: Retorne a média e a variância.
        return mu, logvar
...
class ConditionalDecoder(nn.Module):
    def __init__(self, latent_dim, num_classes, output_channels):
        ...
        # Parte a ser completada pelos alunos:
        # Defina a camada totalmente conectada que recebe 'latent_dim + num_classes' entradas
        self.fc = nn.Linear(latent_dim + num_classes, 128 * 3 * 3)

        # Parte a ser completada pelos alunos:
        # Defina a primeira camada de convolução transposta.
        # Ela deve ter 128 canais de entrada e 64 canais de saída.
        self.deconv1 = nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1)  # [batch_size, 64, 6, 6]

        # Parte a ser completada pelos alunos:
        # Defina a segunda camada de convolução transposta.
        # Ela deve ter 64 canais de entrada e 32 canais de saída.
        self.deconv2 = nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=0)  # [batch_size, 32, 14, 14]

        # Parte a ser completada pelos alunos:
        # Defina a terceira camada de convolução transposta.
        # Ela deve ter 32 canais de entrada e 'output_channels' canais de saída.
        self.deconv3 = nn.ConvTranspose2d(32, output_channels, kernel_size=4, stride=2, padding=1)  # [batch_size, 1, 28, 28]

    def forward(self, z, labels):
        # Parte a ser completada pelos alunos:
        # Concatene a entrada z com os labels ao longo da dimensão correta.
        z = torch.cat((z, labels), dim=1)  # concatenando as labels
        
        # Parte a ser completada pelos alunos:
        # Passe pela camada totalmente conectada.
        h = self.fc(z)
        
        # Parte a ser completada pelos alunos:
        # Redimensione a saída para [batch_size, 128, 3, 3].
        h = h.view(-1, 128, 3, 3)

        # Parte a ser completada pelos alunos:
        # Aplique a primeira camada de convolução transposta e uma função de ativação ReLU.
        h = torch.relu(self.deconv1(h))

        # Parte a ser completada pelos alunos:
        # Aplique a segunda camada de convolução transposta e ReLU.
        h = torch.relu(self.deconv2(h))

        # Parte a ser completada pelos alunos:
        # Aplique a terceira camada de convolução transposta e a função de ativação sigmoid.
...
def train(args):
    ...
    ### Inicialize o modelo, otimizador e função de perda
...
def _get_train_data_loader(batch_size, training_dir, **kwargs):
    """
    Buscando o conjunto de dados de treinamento e criando um DataLoader.
    """
...
def predict_fn(input_data, model):
    ...
    with torch.no_grad():
        # Gerar vetores aleatórios para a predição
